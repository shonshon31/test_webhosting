...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $conn->prepare("INSERT INTO amenity_reservations (first_name, middle_name, last_name, contact_number, email, handle, sources, source_other, package_category, package, check_in_date, check_in_time, guests, total_cost, deposit_amount, remaining_balance, payment_method, proof_of_payment) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("sssissssssssidddss", $first_name, $middle_name, $last_name, $contact_number, $email, $handle, $sources, $source_other, $package_category, $package, $check_in_date, $check_in_time, $guests, $total_cost, $deposit_amount, $remaining_balance, $payment_method, $proof_of_payment);
                    ...
                    <nav class='main-nav'>
                        <ul class='nav-links'>
                            ...
                            foreach ($navLinks as $name => $link) {
                                ...
                                $isActive = ($currentPage == $linkBase || ($currentPage == 'SubmitAmenityReservation.php' && $linkBase == 'AmenitiesPage.php')) ? 'active' : '';
